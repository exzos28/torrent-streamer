...
- ✅ `AddTorrentUseCase` - use case для добавления торрента
- ✅ `RemoveTorrentUseCase` - use case для удаления торрента
- ✅ `ListTorrentsUseCase` - use case для получения списка торрентов
...
- ✅ Обновлены контроллеры для использования новых use cases
- ✅ Созданы тесты для всех новых use cases (12 тестов)
- ✅ Добавлен POST /torrent endpoint для явного добавления торрентов
- ✅ Обновлены e2e тесты (теперь 20 тестов вместо 16)

**Результат:** Бизнес-логика полностью изолирована в use cases, контроллеры стали тоньше и проще тестировать.
...
6. ✅ **Тестирование** - 43 теста (23 unit + 20 e2e), все проходят
7. ✅ **Бизнес-логика в use cases** - вся логика изолирована, контроллеры тонкие
...
│   └── use-cases/       # StreamVideoUseCase, GetTorrentInfoUseCase, AddTorrentUseCase, RemoveTorrentUseCase, ListTorrentsUseCase
...
### Тесты:
- ✅ Unit тесты: 
  - `StreamVideoUseCase.test.ts` (5 тестов)
  - `AddTorrentUseCase.test.ts` (5 тестов)
  - `RemoveTorrentUseCase.test.ts` (4 теста)
  - `ListTorrentsUseCase.test.ts` (3 теста)
  - `ConsoleLogger.test.ts` (6 тестов)
- ✅ E2E тесты: `e2e.test.ts` (20 тестов для всех endpoints)
- ✅ Всего: 43 теста (23 unit + 20 e2e), все проходят
